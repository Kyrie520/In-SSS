...
No stochasticity is introduced into the model (line 119).
"""
...
class Model:
    ...
    # forward propagation
    def enter_network(self, x, y):  # fully connected network
        global w1, w2, w3, w4
        ...
        def save_csv(csv_list, epoch_for_mode, file_name):
            ...
            with open(self.save_path + "\\" + file_name + ".csv", mode, newline="", encoding="utf-8") as f:
                ...
                for item in csv_list:
                    write.writerow([item])
        if epoch == 199:
            save_csv(w1.numpy().flatten().tolist(), epoch, "w1-200")
            save_csv(w2.numpy().flatten().tolist(), epoch, "w2-200")
            save_csv(w3.numpy().flatten().tolist(), epoch, "w3-200")
            save_csv(w4.numpy().flatten().tolist(), epoch, "w4-200")
