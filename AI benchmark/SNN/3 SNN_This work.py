...
class SNN_Stochastic_strategy1(Model):
    def __init__(self):
        super(SNN_Stochastic_strategy1, self).__init__()
        self.flatten = layers.Flatten()
        self.fc1 = DropConnect_noise1(units=2000, rate=0.1)  # keep rate 0.2
        self.lif1 = LeakySurrogate(beta=0.9, p_neuron=1)
        self.fc2 = DropConnect_noise2(units=10, rate=1)
...
history = net.fit(train_dataset, epochs=300, callbacks=[print_accuracy], validation_data=test_dataset, verbose=0)
...
save_path = 'accuracy of SNN_This work.csv'
