"""
Stochastic computing based on the ideal DropConnect synapse
No noise is introduced into the model.
The stochasticity is only used during forward propagation of training (line 141).
The retention probabilities of weights for each layer are set to 0.45, 0.45, 0.80 and 1, respectively (line 325).
"""
    ...
    if type(matrix) == list:
        if not os.path.exists('%s.csv' % name):
            with open('%s.csv' % name, 'w', newline='') as file:
                ...
                writer.writerow(['training accuracy', 'test accuracy', 'w1', 'w2', 'w3', 'w4 update count'])
...
class Model:
    def __init__(self, network_node, p1, p2, p3, p4):
        ...
        self.save_path = 'model_Ideal DropConnect synapse\\device_network_{0}_{1}_{2}_{3}'.format(network_node[0], network_node[1], network_node[2], network_node[3], network_node[4])
    ...
    def enter_network(self, x, y):  # drop-connected network
        ...
        w1 = w1 * mask1 / self.p1  # the stochasticity is used during forward propagation
...
if __name__ == '__main__':
    ...
    model = Model(network_node, 0.45, 0.45, 0.80, 1)  # stochastic computing (p1=0.45, p2=0.45, p3=0.8, p4=1)
    ...
    for epoch in range(epoch):
        ...
        write_csv(model.save_path + '\\benchmark', [accuracy1, accuracy2, model.w1_changes.numpy(), model.w2_changes.numpy(), model.w3_changes.numpy(), model.w4_changes.numpy()])
...
np.savetxt("model_Ideal DropConnect synapse/confusion_matrix.txt", confusion_mtx, fmt="%d")
total = 0
