'''
Stochastic computing with Strategy 1 (this work)
The retention probabilities of weights for each layer are set to 0.45, 0.45, 0.8 and 1, respectively. (line 287)
The stochasticity is only turned on during forward propagation. (line 131, line 134, line 137)
'''
import os
import csv
import time
...
class Model:
    def __init__(self, network_node, p1, p2, p3, p4):
        ...
        self.save_path = 'model_SC1\\device_network_{0}_{1}_{2}_{3}'.format(network_node[0], network_node[1], network_node[2], network_node[3], network_node[4])
...
np.savetxt("confusion_matrix_SC1.txt", confusion_mtx, fmt="%d")
total = 0
for index in range(10):
    total += confusion_mtx[index][index]
print(total)
print("The confusion matrix is saved as a text file：confusion_matrix_SC1.txt")
