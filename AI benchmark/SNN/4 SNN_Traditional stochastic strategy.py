...
The stochasticity being turned on not only in the forward phase, but also in the weight updating phase.
"""
import os
import csv
import numpy as np
...
class SNN_DropConnect(Model):
    def __init__(self):
        ...
        self.fc1 = DropConnect(units=2000, rate=0.5)  # keep rate
...
class CustomAdam(optimizers.Adam):
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        if 'fc2' in var.name:
            new_value = var - learning_rate * 0.5 * grad
        else:
...
history = model.fit(train_dataset, epochs=1000, callbacks=[print_accuracy], validation_data=test_dataset, verbose=0)
