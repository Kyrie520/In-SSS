...
class SNN_DropConnect(Model):
    def __init__(self):
        ...
        self.fc1 = DropConnect1(units=2000, rate=0.2)  # keep rate
        self.lif1 = LeakySurrogate(beta=0.9, p_neuron=1)
        self.fc2 = DropConnect2(units=10, rate=1)
...
class CustomAdam(optimizers.Adam):
    def __init__(self, learning_rate=0.01, name='CustomAdam', **kwargs):
        super(CustomAdam, self).__init__(learning_rate=learning_rate, name=name, **kwargs)

    def _resource_apply_dense(self, grad, var, apply_state=None):
        learning_rate = tf.convert_to_tensor(self.learning_rate, var.dtype.base_dtype)
        if 'fc2' in var.name:
            new_value = var - learning_rate * 0.2 * grad
        else:
            new_value = var - learning_rate * grad
        return tf.compat.v1.assign(var, new_value)
...
history = net.fit(train_dataset, epochs=1000, callbacks=[print_accuracy], validation_data=test_dataset, verbose=0)
...
save_path = 'accuracy of SNN_Traditional stochastic strategy.csv'
...
# # Save model with custom object
# model_save_path = 'snn_dropconnect.h5'
# custom_objects = {'CustomAdam': CustomAdam}
# tf.keras.models.save_model(net, model_save_path, overwrite=True, custom_objects=custom_objects)
