...
The retention probability of weights for each layer is set to 1. (line 286)
'''
import os
import csv
import time
...
class Model:
    def __init__(self, network_node, p1, p2, p3, p4):
        ...
        self.save_path = 'model_DC\\device_network_{0}_{1}_{2}_{3}'.format(network_node[0], network_node[1], network_node[2], network_node[3], network_node[4])
    ...
    def enter_network(self, x, y):  # drop-connected network
        ...
        w1 = w1 * mask1 / self.p1  # the stochasticity is turned off during forward propagation (p1=1, p2=1, p3=1, p4=1)
    ...
    def loss_gradient(self, x, y)  :
        ...
        # back propagation
        self.wp1.assign_sub(lr * cal_gradient(self.wp1, grad_variables[0])*1)  # the stochasticity is turned off during backpropagation
        self.wp2.assign_sub(lr * cal_gradient(self.wp2, grad_variables[2])*1)
        self.wp3.assign_sub(lr * cal_gradient(self.wp3, grad_variables[4])*1)
        self.wp4.assign_sub(lr * cal_gradient(self.wp4, grad_variables[6]))
        self.wm1.assign_sub(-lr * cal_gradient(self.wm1, grad_variables[0])*1)
        self.wm2.assign_sub(-lr * cal_gradient(self.wm2, grad_variables[2])*1)
        self.wm3.assign_sub(-lr * cal_gradient(self.wm3, grad_variables[4])*1)
...
if __name__ == '__main__':
    ...
    model = Model(network_node, 1, 1, 1, 1)  # deterministic computing
...
np.savetxt("confusion_matrix_DC.txt", confusion_mtx, fmt="%d")
total = 0
for index in range(10):
    total += confusion_mtx[index][index]
print(total)
print("The confusion matrix is saved as a text file：confusion_matrix_DC.txt")
