...
The noise is introduced into the model (line 142).
The stochasticity is only used during forward propagation of training (line 144).
The retention probabilities of weights for each layer are set to 0.45, 0.45, 0.80 and 1, respectively (line 346).
"""
...
class Model:
    ...
    def save_weights(self, epoch):
        ...
        if epoch == 199:
            save_csv(w1_mask.numpy().flatten().tolist(), epoch, "w1-200")
            save_csv(w2_mask.numpy().flatten().tolist(), epoch, "w2-200")
            save_csv(w3_mask.numpy().flatten().tolist(), epoch, "w3-200")
            save_csv(w4_mask.numpy().flatten().tolist(), epoch, "w4-200")
