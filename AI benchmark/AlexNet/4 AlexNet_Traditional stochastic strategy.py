...
class AlexNet_DropConnect(Model):
    def __init__(self):
        ...
        self.fc1 = DropConnect(units=512, rate=1, activation='relu')
        self.fc2 = DropConnect(units=128, rate=0.1, activation='relu')  # keep rate
...
class CustomSGD(tf.keras.optimizers.Optimizer):
    ...
    def _resource_apply_dense(self, grad, var):
        learning_rate = tf.convert_to_tensor(self.learning_rate,
                                             name="learning_rate")
        if 'fc2' in var.name:
            tf.compat.v1.assign(var, var - learning_rate * 0.1 * grad)
        else:
